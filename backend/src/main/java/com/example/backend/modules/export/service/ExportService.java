package com.example.backend.modules.export.service;

import com.example.backend.modules.export.dto.ExportRequest;
import com.example.backend.modules.export.entity.Export;
import com.example.backend.modules.export.repository.ExportRepository;
...
public class ExportService {
    ...
    public Export createExport(ExportRequest request) {
        ...
        Product product = productRepository.findByRef(request.getProductRef())
                .orElseThrow(() -> new EntityNotFoundException("Product not found: " + request.getProductRef()));
    ...
    public Export updateExport(Long id, ExportRequest request) {
        ...
        if (!existingExport.getProduct().getRef().equals(request.getProductRef())) {
            product = productRepository.findByRef(request.getProductRef())
                    .orElseThrow(() -> new EntityNotFoundException("Product not found: " + request.getProductRef()));
        }
        ...
        existingExport.setReason(request.getReason());
