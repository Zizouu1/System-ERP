...
export default function PsfOutgoingPage() {
  ...
  async function saveEdit() {
    ...
    try {
      await updateOutgoing(editRow.id, {
        ...
        operationDate: eOperationDate
          ? new Date(eOperationDate).toISOString()
          : undefined,
      });
      setEditOpen(false);
      await refresh();
  ...
  return (
    <div className="page mobilePage">
      <div className="grid">
        ...
        <Card title="Historique des sorties departement 1">
          <TableToolbar
            exportFilename="psf_outgoing"
            exportTitle="Historique des sorties PSF"
            onRefresh={refresh}
            rows={
            ...
            columns={[
              ...
              { header: "Quantité", cell: (h) => h.quantityOut },
              {
                ...
                cell: (h) =>
                  h.operationDate
                    ? new Date(h.operationDate).toLocaleString("fr-FR")
                    : "-",
              },
            ]}
            filterContent={
